...
export function ReelItem({ reel, isActive, onJoinLive }: ReelItemProps) {
  ...
  // Prefetch stream metadata when this reel becomes the active card to reduce join latency
  useEffect(() => {
    if (!isActive || !reel.isLive) return;

    let cancelled = false;

    (async () => {
      try {
        await fetch(`/api/live-streams?channelName=${encodeURIComponent(reel.id)}`, { cache: "no-store" });
      } catch (e) {
        if (!cancelled) {
          // non-fatal; we only warm the cache
        }
      }
    })();
  ...
  return (
    <div className="relative h-screen w-full snap-start bg-black">
      ...
      <div className="absolute top-12 left-4 right-20 z-10 flex items-center gap-3">
        <Image src={reel.creatorAvatar || "/images/avatar-fallback.png"} alt="" width={40} height={40} className="rounded-full border-2 border-white object-cover" unoptimized />
