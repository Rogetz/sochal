...
interface AgoraLiveStreamInnerProps {
  channelName: string;
  role: "host" | "audience";
  userName: string;
  onEnd: () => void;
...
export default function AgoraLiveStreamInner({ channelName, role, userName, onEnd }: AgoraLiveStreamInnerProps) {
      ...
      <div className="relative w-full h-full">
        ...
        <div className="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/60 to-transparent z-10">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button onClick={copyInviteLink} className="size-8 rounded-full bg-black/50 flex items-center justify-center">
        ...
        {role === "host" && isJoined && (
          <div className="absolute bottom-24 left-0 right-0 flex justify-center gap-4 z-10">
            <button onClick={toggleAudio} className="size-12 rounded-full bg-black/60 backdrop-blur flex items-center justify-center hover:bg-black/80 transition">
              {isAudioEnabled ? <Mic className="size-6 text-white" /> : <MicOff className="size-6 text-white" />}
            </button>
            <button onClick={toggleVideo} className="size-12 rounded-full bg-black/60 backdrop-blur flex items-center justify-center hover:bg-black/80 transition">
