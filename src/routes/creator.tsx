...
function CreatorStudio() {
  const { wallet, profile, streams, challenges, selectedChallenge } = useSochal();
  ...
  // Challenges created by this creator
  const myChallenges = useMemo(
    () => challenges.filter((c) => c.creatorId === wallet?.address),
    [challenges, wallet]
  );

  // Available challenges to join (not created by this creator, not full, not completed)
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 md:px-8 py-8 md:py-12">
      ...
      {/* ACTION BUTTONS - Create Reel & Go Live */}
      ...
      {showDashboard && (
        <>
          ...
          {/* AVAILABLE CHALLENGES TO JOIN - Simple Challenge Banners (NO REELS) */}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-4">
                        {/* Placeholder Challenge Image/Avatar */}
                        <div className="size-16 rounded-xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center group-hover:scale-105 transition-transform">
                          <span className="text-3xl">
                            ...
                            {challenge.topic === "Cooking" && "🍳"}
                            {!["Singing","Dancing","Comedy","Rap","Gaming","Cooking"].includes(challenge.topic) && "🏆"}
                          </span>
                        </div>
      ...
      <GoLiveModal
        ...
        onJoinExisting={() => {
          ...
          // Scroll to available challenges section
          setTimeout(() => {
